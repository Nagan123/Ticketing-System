...
<!-- Assign User Modal: -->
<div class="modal" id="assign-user">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form class="z-form" action="<?php admin_action( 'support/assign_user' ); ?>" method="post">
        <input type="hidden" name="id" value="<?php echo intval( $ticket->id ); ?>">
        <div class="modal-body">
          <div class="response-message"></div>
          <p class="mb-2"><?php echo lang( 'sure_assign_user' ); ?></p>
          <select class="form-control select2 search-disabled" data-placeholder="<?php echo lang( 'select_user' ); ?>" name="user" required>
            ...
            <?php if ( ! empty( $users = get_team_users() ) ) {
              ...
              <option value="<?php echo html_escape( $user->id ); ?>" <?php echo select_single( $ticket->assigned_to, $user->id ); ?>><?php echo html_escape( $user->first_name . ' ' . $user->last_name ); ?> ( <?php echo html_escape( $user->username ); ?> )</option>
            <?php }
            } ?>
